...
1. **blue[Reduced the number of training epochs (25)]**: Helps prevent overfitting by limiting the model's exposure to the training data. Stopping the training earlier can improve the model's ability to generalize.
2. **blue[Chose the Exponential Linear Unit (ELU) activation function]**: ELU can capture both positive and negative input regions (unlike ReLU which sets all negative input to 0) and returns non-zero outputs for negative input. By providing non-zero gradients for both positive and negative values, ELU helps maintain a more balanced and stable gradient flow throughout the network.
3. **blue[Added L2 regularizer]**: The L2 regularizer encourages the weights to be small, which reduces the model's complexity and prevents hte model from becoming too specialized to the training data (better generalized performance).
4. **blue[Increased Dropout rate (0.6)]**: We increased the drop out rate to reduce the model's reliance on specific inputs/features and encourages generalization.
''')
model = load()
