...
1. <mark>Reduced the number of training epochs (25)</mark>: Helps prevent overfitting by limiting the model's exposure to the training data. Stopping the training earlier can improve the model's ability to generalize.
2. <mark>Chose the Exponential Linear Unit (ELU) activation function</mark>: ELU can capture both positive and negative input regions (unlike ReLU which sets all negative input to 0) and returns non-zero outputs for negative input. By providing non-zero gradients for both positive and negative values, ELU helps maintain a more balanced and stable gradient flow throughout the network.
3. <mark>Added L2 regularizer</mark>: The L2 regularizer encourages the weights to be small, which reduces the model's complexity and prevents hte model from becoming too specialized to the training data (better generalized performance).
4. <mark>Increased Dropout rate (0.6)</mark>: We increased the drop out rate to reduce the model's reliance on specific inputs/features and encourages generalization.
''')
model = load()
